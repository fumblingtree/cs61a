...
def is_swap(score0, score1):
    ...
    # BEGIN PROBLEM 4 #true false; including 0,1
    if score0 == 0 and score1 // 10 == 0: 
        return True 
    elif score1 // 10 == score0 % 10: 
        return True
    else:
        return False 
    # END PROBLEM 4
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    def speak(score0, score1):
        buffer_score = previous_score 
        buffer_high = previous_high 
        if who == 1:
            my_score = score1
        else: 
            my_score = score0
        gap = my_score - buffer_score
        if buffer_score != my_score: 
            if gap > buffer_high:    
                buffer_high = gap
                print(buffer_high, "point(s)! That's the biggest gain yet for Player", who) 
            buffer_score = my_score
        return announce_highest(who, buffer_high, buffer_score)
    return speak
    # END PROBLEM 7
...
def make_averaged(fn, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def make_and_return(*args): 
        result = 0
        for i in range(0, num_samples):  
            i += 1
            result += fn(*args)
        return result / num_samples 
    return make_and_return 
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    store, average = 0, 0
    while times <= 10: 
        temp_func = make_averaged(roll_dice, num_samples)
        store = temp_func(times, dice) 
        if average < store: 
            average = store 
            numbers = times 
        times += 1
